...
@chara アオイ right 笑顔  ← 立ち絵を表示(背景とは別レイヤー)
...
| `@chara 名前 [位置] [表情]` | 立ち絵を表示(`chara/` フォルダ)。`@show` も同じ |
| `@face 名前 表情` | 表情だけ差し替え(立ち絵はそのまま) |
...
立ち絵は**背景とは別レイヤー**で、`chara/` フォルダから読み込みます。
表情は「ベース画像＋顔差分」を重ねて表現します。

```
chara/アオイ.png        ← ベース(体・輪郭)
chara/アオイ_笑顔.png    ← 表情差分(顔だけ。透過PNGでベースに重なる)
chara/アオイ_泣き.png    ← 別の表情差分
```

```
@chara アオイ right 笑顔   ← 右にアオイを「笑顔」で表示
@chara ユウタ left         ← 左にユウタを表示(表情なし=ベースのみ)
@face アオイ 泣き           ← アオイの表情だけ「泣き」に差し替え
@hide アオイ               ← アオイだけ退場
@hide all                 ← 全員退場
```

- 位置は `left` / `center` / `right`(`左` `中` `右` も可)。省略時は既存の位置を維持。
- 喋っているキャラの立ち絵が明るくなり、それ以外は自動で少し暗くなります。
- 立ち絵はセーブデータとシーン鑑賞スナップショットに含まれ、ロード/再生時に復元されます。
...
- **立ち絵・表情差分** — 背景と別レイヤー。ベース＋顔差分の重ね合わせ、話者ハイライト
- **CGギャラリー** — `@cg` の一枚絵を自動登録。未見は「?」表示
- **シーン鑑賞モード** — `@scene` 到達で解放。その時点のフラグ・背景・立ち絵・BGMごと復元して再生
